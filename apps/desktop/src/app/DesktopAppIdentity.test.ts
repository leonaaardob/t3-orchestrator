...
describe("DesktopAppIdentity", () => {
  it.effect("always uses the isolated Orchestrator userData path", () =>
    withIdentity(
      Effect.gen(function* () {
        const identity = yield* DesktopAppIdentity.DesktopAppIdentity;
        const userDataPath = yield* identity.resolveUserDataPath;

        assert.equal(userDataPath, "/Users/alice/Library/Application Support/T3 Orchestrator");
      }),
    ),
  );
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["T3 Orchestrator"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "T3 Orchestrator");
