...
const makeDesktopClerkLayer = (isDevelopment = true, events: string[] = []) => {
  const environment = DesktopEnvironment.DesktopEnvironment.of({
    ...
    userDataDirName: isDevelopment ? "T3 Orchestrator (Dev)" : "T3 Orchestrator",
...
describe("DesktopClerk", () => {
  ...
  it.effect("acquires and releases the SDK bridge with the layer", () => {
    ...
    return Effect.gen(function* () {
        ...
        [
          {
            ...
            passkeys: true,
            renderer: { scheme: "t3orchestrator-dev", host: "app" },
          },
        ],
      ]);
      ...
      assert.deepEqual(events, [
        "setPath:userData:/tmp/app-data/T3 Orchestrator (Dev)",
        "createClerkBridge",
      ]);
  ...
  it.each([
    { isDevelopment: true, scheme: "t3orchestrator-dev" },
    { isDevelopment: false, scheme: "t3orchestrator" },
