...
describe("DesktopEnvironment", () => {
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.appUserModelId, "com.t3orchestrator.app.dev");
      assert.equal(environment.linuxWmClass, "t3-orchestrator-dev");
      assert.deepEqual(
  ...
  it.effect("keeps implicit development state separate from production state", () =>
    Effect.gen(function* () {
      ...
      assert.equal(development.stateDir, "/Users/alice/.t3-orchestrator/dev");
      assert.equal(production.stateDir, "/Users/alice/.t3-orchestrator/userdata");
    }),
  );
