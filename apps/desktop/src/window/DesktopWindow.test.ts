...
function makeFakeBrowserWindow() {
  ...
  const webContents = {
    copyImageAt: vi.fn(),
    getURL: vi.fn(() => "t3orchestrator-dev://app/"),
  ...
  it.effect("does not open a development window until the backend is ready", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        assert.deepEqual(fakeWindow.loadURL.mock.calls[0], ["t3orchestrator-dev://app/"]);
  ...
  it("retries only transient failures for the development renderer", () => {
    assert.isTrue(
      DesktopWindow.isRetryableDevelopmentRendererLoadFailure({
        applicationUrl: "t3orchestrator-dev://app/",
        errorCode: -102,
        isMainFrame: true,
        validatedUrl: "t3orchestrator-dev://app/",
      }),
    );
    assert.isFalse(
      DesktopWindow.isRetryableDevelopmentRendererLoadFailure({
