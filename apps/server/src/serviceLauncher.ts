...
// `t3-orchestrator service update`. Keep runtime imports limited to Node built-ins.
...
import { formatPackageSpec, pinnedRuntimeEntryPath } from "@t3tools/shared/distributionIdentity";
import type {
...
const runtimePaths = (baseDir: string, version: string) => {
  ...
  return {
    versionDir,
    entryPath: pinnedRuntimeEntryPath(NodePath.join, versionDir),
...
export class Launcher {
  ...
  async #startChild(version: string, role: ChildRole, update?: ServiceUpdateRecord): Promise<void> {
    ...
    if (!(await runtimeExists(this.#baseDir, version))) {
      throw new Error(`Selected ${formatPackageSpec(version)} runtime is missing or incomplete.`);
    }
    if (this.#stopping) return;
