  ...
  return (
    <SettingsPageContainer>
      <SettingsSection id="appearance" title="Appearance">
        ...
        <SettingsRow
          {...searchableSetting("theme-customization")}
          description="Accent color, interface font, and background texture layered over the active theme."
          resetAction={
            customization.accent !== DEFAULT_THEME_CUSTOMIZATION.accent ||
            customization.font !== DEFAULT_THEME_CUSTOMIZATION.font ||
            customization.backgroundEffect !== DEFAULT_THEME_CUSTOMIZATION.backgroundEffect ? (
          ...
          control={
            <div className="grid w-full grid-cols-1 gap-3 sm:grid-cols-3">
              <label className="space-y-1.5">
                <span className="block text-[11px] font-medium text-muted-foreground">Accent</span>
                <Select
                  value={customization.accent}
                  onValueChange={(value) => {
                    if (THEME_ACCENT_OPTIONS.some((option) => option.value === value)) {
                      setThemeCustomization({ accent: value as ThemeAccent });
                    }
                  }}
                >
                  <SelectTrigger className="w-full" aria-label="Theme accent">
                    <SelectValue>
                      {THEME_ACCENT_OPTIONS.find((option) => option.value === customization.accent)
                        ?.label ?? "Default"}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectPopup align="end" alignItemWithTrigger={false}>
              ...
              <label className="space-y-1.5">
                <span className="block text-[11px] font-medium text-muted-foreground">Font</span>
                <Select
                  value={customization.font}
                  onValueChange={(value) => {
                    if (THEME_FONT_OPTIONS.some((option) => option.value === value)) {
                      setThemeCustomization({ font: value as ThemeFont });
                    }
                  }}
                >
                  <SelectTrigger className="w-full" aria-label="Theme font">
                    <SelectValue>
                      {THEME_FONT_OPTIONS.find((option) => option.value === customization.font)
                        ?.label ?? "Default"}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectPopup align="end" alignItemWithTrigger={false}>
              ...
              <label className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    if (THEME_BACKGROUND_EFFECT_OPTIONS.some((option) => option.value === value)) {
                      setThemeCustomization({ backgroundEffect: value as ThemeBackgroundEffect });
                    }
                  }}
                >
                  <SelectTrigger className="w-full" aria-label="Theme background effect">
                    <SelectValue>
                      {THEME_BACKGROUND_EFFECT_OPTIONS.find(
                        (option) => option.value === customization.backgroundEffect,
                      )?.label ?? "Default"}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectPopup align="end" alignItemWithTrigger={false}>
                    {THEME_BACKGROUND_EFFECT_OPTIONS.map((option) => (
                      <SelectItem hideIndicator key={option.value} value={option.value}>
                        {option.label}
                      </SelectItem>
                    ))}
