 ...
 *
 * A linked git worktree gets its own (gitignored) `.t3-orchestrator`: feature work in a
...
import { defaultRemoteHomeName } from "./distributionIdentity.ts";
...
export const resolveWorktreeT3Home = (
  ...
  Effect.gen(function* () {
    ...
    const path = yield* Path.Path;
    return path.join(worktreePath, defaultRemoteHomeName);
  });
