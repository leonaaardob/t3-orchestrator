...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual((linux.linux as Record<string, unknown>).protocols, [
        { name: "T3 Orchestrator", schemes: ["t3orchestrator", "t3orchestrator-dev"] },
      ]);
  ...
  it.effect(
    "uses hardened-runtime Electron entitlements and both renderer protocols for signed macOS builds",
    () =>
      Effect.gen(function* () {
        const config = yield* createBuildConfig("mac", "dmg", "1.2.3", true, false, undefined);

        const mac = config.mac as Record<string, unknown>;
        assert.equal(config.appId, "com.t3orchestrator.app");
        assert.isTrue(mac.hardenedRuntime as boolean);
        assert.isTrue(mac.notarize as boolean);
        assert.match(String(mac.entitlements), /apps\/desktop\/entitlements\.mac\.plist$/);
        assert.match(
          String(mac.entitlementsInherit),
          /apps\/desktop\/entitlements\.mac\.inherit\.plist$/,
        );
