...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("createDevRunnerEnv", () => {
      ...
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          ...
          baseEnv: {
            ...
            T3_BOOT_SERVICE_UNIT: "t3-orchestrator.service",
          },
          serverOffset: 0,
          webOffset: 0,
          t3Home: undefined,
    ...
    // --host configures the backend (T3CODE_HOST). For proxiable bind hosts
    // such as 0.0.0.0 it also pins Vite's HOST so LAN/share HMR stays on the
    // same interface; non-proxiable values are rejected earlier by the runner.
    it.effect("pins Vite HOST for proxiable --host values and always sets T3CODE_HOST", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: { HOST: "127.0.0.1" },
          serverOffset: 0,
          webOffset: 0,
          t3Home: undefined,
          browser: undefined,
  ...
  describe("runDevRunnerWithInput", () => {
    ...
    // Base-dir precedence (--home-dir > worktree .t3-orchestrator > ambient T3CODE_HOME)
    ...
    describe("t3 home precedence", () => {
      ...
      it.effect("treats a blank --home-dir as unset rather than as a selection", () =>
        Effect.gen(function* () {
          ...
          assert.equal(home, path.join(path.resolve(root), ".t3-orchestrator"));
        }).pipe(Effect.scoped),
      );

      it.effect("prefers the worktree .t3-orchestrator over an ambient T3CODE_HOME", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const root = yield* makeWorktree;
          const home = yield* spawnedHome({
