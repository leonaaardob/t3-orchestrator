...
import { defaultRemoteHomeName } from "@t3tools/shared/distributionIdentity";
...
export const DEFAULT_T3_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), defaultRemoteHomeName),
);
...
const MODE_ARGS = {
  ...
  "dev:server": ["run", "--filter=t3-orchestrator", "dev"],
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .t3-orchestrator > ambient T3CODE_HOME) is resolved
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
    ...
    // (gitignored) `.t3-orchestrator` — see @t3tools/shared/devHome for why this must
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit T3 Code data directory; runtime state is stored under userdata (equivalent to T3CODE_HOME). Inside a git worktree this defaults to that worktree's own .t3-orchestrator so dev state stays off the shared home.",
    ),
    Flag.optional,
